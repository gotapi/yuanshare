...
function initConfig() {
    try {
        ...
        if (config) {
            ...
            console.log("init config:",localConfig.config);
            return true;
        }
        const defalutConfig = {};
        ...
        console.log("error while init storage:")
        console.log(e);
        return false;
    }
}
...
function readConfig() {
    try {
        console.log("read from :",localConfig.configUrl);
...
const localConfig = {
    config: null,
    configUrl: path.join(__dirname,"/.404up.json"),
    init:()=>{
        initConfig();
    },
    sync:()=>{
        readConfig();
        writeConfig(localConfig.config);
